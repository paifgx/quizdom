...
def upgrade() -> None:
    ...
    op.execute("ALTER TYPE gamestatus ADD VALUE IF NOT EXISTS 'waiting' BEFORE 'active'")
    op.execute("ALTER TYPE gamestatus ADD VALUE IF NOT EXISTS 'countdown' AFTER 'waiting'")

    # Add countdown_started_at column to track countdown start time
    op.add_column(
        'gamesession',
        sa.Column('countdown_started_at', sa.DateTime(), nullable=True)
    )

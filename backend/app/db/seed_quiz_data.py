...
def create_test_quiz_data(session: Session) -> None:
    ...
    quizzes_data: List[Dict[str, Any]] = [
        ...
        },
        {
            ...
            "questions": [
                ...
                {
                    "content": "What planet is known for its rings?",
                    "explanation": "Saturn's rings are visible from Earth.",
                    "difficulty": 2,
                    "answers": [
                ...
                {
                    "content": "What gas do plants absorb?",
                ...
                },
                {
                    ...
                    "explanation": "Atomic nucleus contains protons and neutrons.",
                    "difficulty": 3,
                    "answers": [
    ...
    for entry in quizzes_data:
        ...
        t_data = entry["topic"]
        topic = session.exec(
            select(Topic).where(Topic.title == t_data["title"])
        ).first()
        if not topic:
            topic = Topic(title=t_data["title"], description=t_data["description"])
            session.add(topic)
            session.commit()
        ...
        if topic.id is None:
            raise ValueError(f"Topic {topic.title} was not properly saved to database")
        ...
        if not existing_quiz:
            ...
            for idx, question in enumerate(created_questions):
                qq = QuizQuestion(quiz_id=quiz.id, question_id=question.id, order=idx)
                session.add(qq)
            session.commit()

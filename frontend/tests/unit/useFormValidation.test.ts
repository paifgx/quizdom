...
describe('useFormValidation', () => {
  describe('validateField', () => {
    it('validates email correctly', () => {
      ...
      expect(result.current.getError('email')).toBe('Please enter a valid email address')
    ...
    it('validates password correctly', () => {
      ...
      expect(result.current.getError('password')).toBe('Password must be at least 6 characters')
    ...
    it('validates confirmPassword correctly', () => {
      ...
      expect(result.current.getError('confirmPassword')).toBe('Please confirm your password')

      act(() => {
        result.current.validateField('confirmPassword', 'different', 'password123')
      })
      expect(result.current.getError('confirmPassword')).toBe('Passwords do not match')

      act(() => {
        result.current.validateField('confirmPassword', 'password123', 'password123')
      })
    ...
    it('validates firstName correctly', () => {
      ...
      expect(result.current.getError('firstName')).toBe('First name is required')

      act(() => {
        result.current.validateField('firstName', 'J')
      })
      expect(result.current.getError('firstName')).toBe('First name must be at least 2 characters')
    ...
    it('validates lastName correctly', () => {
      ...
      expect(result.current.getError('lastName')).toBe('Last name must be at least 2 characters')
